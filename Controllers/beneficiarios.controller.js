...
const createHistorialClinico = async (req, res) => {
    const { ENFERMEDAD_PADECE, MEDICAMENTOS_INGIERE, VACUNAS, AUDICION, ORFTAMOLOGICAS, APARATO_AUDITIVO, LENTES, CIRUJIAS, OTRAS, DIAGNOSTICO, DISCAPACIDAD, COMORBILIDAD, ADQUIRIDA, TEMPORALIDAD } = req.body;
    const { idbene } = req.params;
    try {
        await sequelize.query(`INSERT INTO HISTORIAL_CLINICO(ID_BENEFICIARIO,ENFERMEDAD_PADECE,MEDICAMENTOS_INGIERE,VACUNAS,AUDICION,ORFTAMOLOGICAS,APARATO_AUDITIVO,LENTES,CIRUJIAS,OTRAS,DIAGNOSTICO,DISCAPACIDAD, COMORBILIDAD, ADQUIRIDA, TEMPORALIDAD) VALUES(${idbene},'${ENFERMEDAD_PADECE}','${MEDICAMENTOS_INGIERE}','${VACUNAS}','${AUDICION}','${ORFTAMOLOGICAS}','${APARATO_AUDITIVO}','${LENTES}','${CIRUJIAS}','${OTRAS}','${DIAGNOSTICO}','${DISCAPACIDAD}','${COMORBILIDAD}','${ADQUIRIDA}','${TEMPORALIDAD}')`, { type: QueryTypes.INSERT });
