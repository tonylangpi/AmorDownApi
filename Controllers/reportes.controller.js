...
const Filtros = (req, res) => {
  ...
  if(Tipo === 1) {
    ...
    connection.query("SELECT CONCAT(B.NOMBRES, ' ', B.APELLIDOS) AS NOMBRE_COMPLETO, B.SEXO, TIMESTAMPDIFF(YEAR, B.FECHA_NACIMIENTO, CURDATE()) AS EDAD, B.CORRELATIVO, E.CODIGO, B.DEPARTAMENTO, B.MUNICIPIO, B.DIRECCION FROM HISTORIAL_CLINICO HC INNER JOIN BENEFICIARIO B ON HC.ID_BENEFICIARIO = B.ID_BENEFICIARIO INNER JOIN EMPRESA E ON B.ID_EMPRESA = E.ID_EMPRESA WHERE HC.LENTES = 'SI' ORDER BY EDAD", [Filtro], (err, result) =>{
      if(err){
        console.log(err)
      } else {
        res.json(result)
    ...
    connection.query("SELECT CONCAT(B.NOMBRES, ' ', B.APELLIDOS) AS NOMBRE_COMPLETO, B.SEXO, TIMESTAMPDIFF(YEAR, B.FECHA_NACIMIENTO, CURDATE()) AS EDAD, B.CORRELATIVO, E.CODIGO, B.DEPARTAMENTO, B.MUNICIPIO, B.DIRECCION FROM HISTORIAL_CLINICO HC INNER JOIN BENEFICIARIO B ON HC.ID_BENEFICIARIO = B.ID_BENEFICIARIO INNER JOIN EMPRESA E ON B.ID_EMPRESA = E.ID_EMPRESA WHERE HC.APARATO_AUDITIVO = 'SI' ORDER BY EDAD", [Filtro], (err, result) =>{
      if(err){
        console.log(err)
      } else {
        res.json(result)
